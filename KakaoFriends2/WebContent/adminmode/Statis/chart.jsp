...
<!--// mArticle -->
<div id="mArticle" align="center">
	<script src="./resources/js/Chart.js"></script>
<div id=ChartSize style="width:1000px; height: 600px;">
...
<%
	...
	all = (ArrayList)session.getAttribute("all_list");
	allGoods = (ArrayList<GoodsBean>)all.get(0);
	orderGoods = (ArrayList<GoodsOrderBean>)all.get(1);
	members = (ArrayList<MemberBean>)all.get(2);
	
	GoodsOrderBean goodslist = new GoodsOrderBean();
	int goodsChart[] = new int[9];
	for(int i=0; i<orderGoods.size(); i++){
		if(orderGoods.get(i).getGCATEGORY().equals("64")){
			goodsChart[0] = orderGoods.get(i).getGCOUNT();
		}else if(orderGoods.get(i).getGCATEGORY().equals("22")){
			goodsChart[1] = orderGoods.get(i).getGCOUNT();
		}else if(orderGoods.get(i).getGCATEGORY().equals("13")){
			goodsChart[2] = orderGoods.get(i).getGCOUNT();
		}else if(orderGoods.get(i).getGCATEGORY().equals("14")){
			goodsChart[3] = orderGoods.get(i).getGCOUNT();
		}else if(orderGoods.get(i).getGCATEGORY().equals("24")){
			goodsChart[4] = orderGoods.get(i).getGCOUNT();
		}else if(orderGoods.get(i).getGCATEGORY().equals("15")){
			goodsChart[5] = orderGoods.get(i).getGCOUNT();
		}else if(orderGoods.get(i).getGCATEGORY().equals("17")){
			goodsChart[6] = orderGoods.get(i).getGCOUNT();
		}else if(orderGoods.get(i).getGCATEGORY().equals("16")){
			goodsChart[7] = orderGoods.get(i).getGCOUNT();
		}else if(orderGoods.get(i).getGCATEGORY().equals("115")){
			goodsChart[8] = orderGoods.get(i).getGCOUNT();
		}
	}
	
	int goodsCount[] = new int[9];
	for(int i=0; i<allGoods.size(); i++){
		goodsCount[i] = allGoods.get(i).getGCOUNT();
	}

	int membersCount[]=new int[9];
	for(int i=0;i<membersCount.length;i++){
		membersCount[i]=0;
	}
	
	for(int i=0; i<members.size(); i++){
		String birth=members.get(i).getBIRTH().substring(0, 4);
		int birthday = Integer.parseInt(birth);
		if(birthday<1930){
			membersCount[0]++;
		}else if(birthday<1940){
			membersCount[1]++;
		}else if(birthday<1950){
			membersCount[2]++;
		}else if(birthday<1960){
			membersCount[3]++;
			
		}else if(birthday<1970){
			membersCount[4]++;
			
		}else if(birthday<1980){
			membersCount[5]++;
			
		}else if(birthday<1990){
			membersCount[6]++;
...
var myChart = new Chart(ctx, {
    type: 'bar',
    data: {
        labels: ["인형/피규어", "리빙", "잡화", "의류", "쥬얼리", "문구", "여행/레져", "생활테크", "NINIZ"],
        datasets: [{
            label: '# 판매현황',
            data: [<%=goodsChart[0]%>, <%=goodsChart[1]%>, <%=goodsChart[2]%>, <%=goodsChart[3]%>, <%=goodsChart[4]%>, <%=goodsChart[5]%>, <%=goodsChart[6]%>, <%=goodsChart[7]%>, <%=goodsChart[8]%>],
            backgroundColor: [
                'rgba(255, 99, 132, 0.2)',
                'rgba(54, 162, 235, 0.2)',
                'rgba(255, 206, 86, 0.2)',
...
var myChart = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            label: '# 재고현황',
            data: [<%=goodsCount[1]%>, <%=goodsCount[6]%>, <%=goodsCount[3]%>, <%=goodsCount[0]%>, <%=goodsCount[4]%>, <%=goodsCount[7]%>, <%=goodsCount[2]%>, <%=goodsCount[8]%>, <%=goodsCount[5]%>],
            backgroundColor: [
                'rgba(255, 99, 132, 0.2)',
                'rgba(54, 162, 235, 0.2)',
                'rgba(255, 206, 86, 0.2)',
...
var myChart = new Chart(ctx, {
    ...
    data: {
        ...
        datasets: [{
            label: '# 회원현황',
            data: [<%=membersCount[8]%>, <%=membersCount[7]%>, <%=membersCount[6]%>, <%=membersCount[5]%>, <%=membersCount[4]%>, <%=membersCount[3]%>, <%=membersCount[2]%>, <%=membersCount[1]%>, <%=membersCount[0]%>],
            backgroundColor: [
                'rgba(255, 99, 132, 0.2)',
                'rgba(54, 162, 235, 0.2)',
                'rgba(255, 206, 86, 0.2)',
