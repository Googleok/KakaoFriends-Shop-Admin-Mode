...
<style type="text/css">
	...
	a:link.customlist, a:visited.customlist {
    ...
    a:hover.customlist, a:active.customlist { background-color: #FF4500; }
    
</style>
<!--// mArticle -->
<div id="mArticle" align="center">

	<table border="1" style="margin-top: ;">
	<%
	ArrayList result=(ArrayList)session.getAttribute("result");
	for(int i=0; i<result.size(); i++){
	%>
		<tr>
			<td style="color: ; font-size: 30px;">멤버명</td>
			<td><a href="Member_info.mo?userid=<%=result.get(i)%>" style="background-color: aqua;"><%=result.get(i)%></a></td>
			<td><a href="Member_delete.mo?userid=<%=result.get(i)%>" class="customlist">삭제</a></td>
		</tr>
	<%
	}
	%>	
